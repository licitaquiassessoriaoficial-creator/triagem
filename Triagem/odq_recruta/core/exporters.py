...
def export_csv(aprovados: list[CandidateResult], path: Path):
    with open(path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["email_id", "remetente", "assunto", "nome_arquivo", "hash", "score", "caminho_arquivo", "data_email"])
        for c in aprovados:
            a = c.attachment
            writer.writerow([
                a.email_id, a.sender, a.subject, a.filename, a.hash, c.score, str(a.path), a.received_at
            ])
