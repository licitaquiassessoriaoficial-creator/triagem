...
class MainWindow:
    ...
    def __init__(self, root):
        ...
        # Descrição da vaga
        tk.Label(frame, text="Descrição da vaga:").grid(
        ...
        # Palavras-chave
        tk.Label(
            frame, text="Palavras-chave (essenciais, separadas por vírgula):"
        ).grid(row=1, column=0, sticky="w")
        self.entry_keywords = tk.Entry(frame, width=50)
        self.entry_keywords.grid(row=1, column=1, pady=5)

        tk.Label(frame, text="Desejáveis (opcional, vírgula):").grid(
        ...
        # Threshold
        tk.Label(frame, text="Threshold (0-100):").grid(
        ...
        tk.Checkbutton(frame, text="Usar Gmail", variable=self.var_gmail).grid(
            row=3, column=0, sticky="w"
        )
        tk.Checkbutton(
            frame, text="Usar Microsoft 365", variable=self.var_m365
        ).grid(row=3, column=1, sticky="w")

        # Botões
        self.btn_iniciar = tk.Button(
            frame, text="Iniciar", width=12, command=self.on_iniciar
        )
        ...
        self.btn_exportar = tk.Button(
            frame, text="Exportar Relatório", width=18
        )
        ...
        self.txt_logs = scrolledtext.ScrolledText(
            frame, height=10, width=100, state="disabled"
        )
    ...
    def on_iniciar(self):
        ...
        try:
            ...
            self.update_logs(
                "[ERRO] Threshold deve ser um número entre 0 e 100."
            )
            return
        if not outdir:
    ...
    def start_pipeline(self):
        ...
        try:
            ...
            keywords = [
                k.strip()
                for k in self.entry_keywords.get().split(",")
                if k.strip()
            ]
            desired = [
                k.strip()
                for k in self.entry_desired.get().split(",")
                if k.strip()
            ]
            ...
            def run_and_update():
                ...
                if use_m365:
                    self.update_logs(
                        "[INFO] Buscando anexos no Microsoft 365..."
                    )
                ...
                aprovados_count = sum(
                    1 for c in pipeline.aprovados if c.approved
                )
