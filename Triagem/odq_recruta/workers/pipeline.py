...
from core.models import Attachment, CandidateResult, JobSpec
...
class Pipeline:
    ...
    def run(self):
        ...
        self.log_queue.put(
            f"[INFO] Total de anexos encontrados: {len(attachments)}"
        )
        # Deduplicação
        new_attachments = [
            a for a in attachments if a["hash"] not in self.hash_cache
        ]
        self.log_queue.put(
            f"[INFO] Anexos novos após deduplicação: {len(new_attachments)}"
        )
